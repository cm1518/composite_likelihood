function [Y_for llk xest] = KF(A,B,C,D,x10,S10,Y, horizon,simulations)
...
Y_for=zeros(N,horizon,simulations);
xest(:,1)=x10;
llk=0;
for i=1:T
    temp=(A*S10*A'+B);
    if any(isnan(temp(:))) || any(isinf(temp(:))) || any(max(abs(temp(:)))>1e20)
        llk=-inf;
        break
    end
...
[S10,k]=nearestSPD(S10);
for ss=1:simulations
xest_temp=mvnrnd(xest(:,T+1),S10)';
%xest_temp=xest(:,T+1);

for hh=1:horizon
   Y_for(:,hh,ss)=A*xest_temp+mvnrnd(zeros(N,1),B)';
   xest_temp=C*xest_temp+mvnrnd(zeros(sizex,1),D)';
  % xest_temp=C*xest_temp; 
