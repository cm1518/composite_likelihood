...
% [IRFS(:,:,mm,1+simulations*(nn-1):simulations*nn)] = IRF_simul3( draws(:,indices(nn)),setup,mm,horizon,data, variables(:,mm), shocks(mm) ,simulations);
% end
% 
% end
get_model_ind_irfs
...
model_irfs5(:,mm,vv)=squeeze(prctile(IRFS(vv,:,mm,:),5,4));
model_irfs95(:,mm,vv)=squeeze(prctile(IRFS(vv,:,mm,:),95,4));
...
disp(nn/number_of_draws)
for mm=1:setup.number_models
[IRFS_pdf{vv}(mm,:,:,nn)] = IRF_density3( draws(:,indices(nn)),setup,mm,horizon,data, variables(vv,mm), shocks(mm),grid_{vv});
end
...
IRF_percentiles(vv,hh,:)= prctile(draws_temp,[5 50 95]);
...
figure;
for mm=1:setup.number_models
plot(1:horizon,model_irfs(:,mm,1),'LineWidth',2)
hold on
end
...
legend('small NK with wages', 'small NK','large NK','search','BGG','CL 5th percentile', 'CL','CL 95th percentile','Location','southeast')
print -depsc
