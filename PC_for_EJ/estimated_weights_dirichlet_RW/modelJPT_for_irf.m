function [zmat, B, C_final, D_final, add_matrices,inv_shock_std]=modelJPT(param,setup,data)
...
% -------------------------------------------------------------------------
numpar=      37;  % Number of parameters 
ncof=        30;  % Number of coefficients not corresponding to 
                  % standard deviations 
...
gss   = 0.22;				  
				  ...
				  % Non SD parameters 
% -------------------------------------------------------------------------
kappa_p=param(1);     % share of capital in the prod. function
%delta=param(2);     % capital depreciation rate
gamma_b=param(2);     % price indexation (=0 is static indexation, =1 is dynamic)
...
%gss=param(12);      % steady state government spending to GDP ratio
niu=param(11);      % inverse Frisch elasticity
alpha=param(12);      % price stickiness
...
% -------------------------------------------------------------------------
GAM0(z,z)=1; GAM1(z,z)=rhoz; PSI(z,zs)=sdz;
GAM0(g,g)=1; GAM1(g,g)=rhog; PSI(g,gs)=sdg;
...
GAM0(ARMAlambdaw,ARMAlambdaw)=1; PSI(ARMAlambdaw,lambdaws)=sdlambdaw;

GAM0(miu,miu)=1; GAM1(miu,miu)=rhomiu; PSI(miu,mius)=sdmiu;
...
GAM0(ARMAlambdap,ARMAlambdap)=1; PSI(ARMAlambdap,lambdaps)=sdlambdap;

GAM0(b,b)=1; GAM1(b,b)=rhob; PSI(b,bs)=sdb;
GAM0(mp,mp)=1; GAM1(mp,mp)=rhomp; PSI(mp,Rs)=sdR;
...
D=impact;
C_final=zeros(size(G1,1)+1,size(G1,1)+1);
C_final(1:size(G1,1),1:size(G1,1))=G1;
C_final(end,end)=1;
C_final(1:end-1,end)=C;
D_final=zeros(size(D,1)+1,size(D,2)+1);
 D_final(1:size(D,1),1:size(D,2))=D;

inv_shock_std=1/impact(10,1);
